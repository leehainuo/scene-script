...
func DeleteScriptHandler(svc *svc.ServiceContext) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		req := &types.DeleteScriptReq{ID: httpn.PathString(c, "id")}
		if err := httpn.Validate(req); err != nil {
			httpn.Error(c, err)
			return
		}

		l := script.NewDeleteScriptLogic(c, svc)
		resp, err := l.Delete(userID, req)
		if err != nil {
